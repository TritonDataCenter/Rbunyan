...
#    cat("NOT LOGGED\n")
    if (bunyan_globals$jsonout == TRUE) {
      return("")
    } else {
      return()
    }
...
bunyanLog.trace <- 
function(level, msg, req, res, version, json=FALSE) {
...
bunyanLog.info <- 
function(level, msg, req, res, version, json=FALSE) {
  bunyanLog(msg=msg, level=30, req=req, res=res, version=version)
}

bunyanLog.warn <- 
function(level, msg, req, res, version, json=FALSE) {
  bunyanLog(msg=msg, level=40, req=req, res=res, version=version)
}
...
bunyanLog.fatal <- 
  function(level, msg, req, res, version) {
